...
class NetworkControl:
    ...
    def parse_mem(self, val):
        # Supports values like '17.79MiB', '7.756GiB', '126B', etc.
    ...
    def parse_mem_pair(self, val):
        # For values like '17.79MiB / 7.756GiB'
        parts = val.split('/')
        if len(parts) == 2:
            return (self.parse_mem(parts[0]), self.parse_mem(parts[1]))
        return (self.parse_mem(val), 0.0)

    def parse_io_pair(self, val):
        # For values like '24.9kB / 5.33kB'
        parts = val.split('/')
        if len(parts) == 2:
            return (self.parse_mem(parts[0]), self.parse_mem(parts[1]))
        return (self.parse_mem(val), 0.0)
